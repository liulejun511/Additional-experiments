...
class ECRTM(nn.Module):
    ...
    def topic_diversity_loss(self, beta):
        b = F.normalize(beta, p=2, dim=1)
        cos = b @ b.T
        off = cos - torch.eye(cos.size(0), device=cos.device)
        return (off ** 2).mean()
    ...
    def forward(self, input):
        ...
        loss_div = self.topic_diversity_loss(beta)*5   # scalar

        loss = loss_TM + loss_ECR + loss_div
