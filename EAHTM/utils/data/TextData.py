...
class TextData:
    def __init__(self, dataset_path, batch_size, device, embedding_type='current', embedding_path=''):
        self.train_data, self.test_data, self.vocab, self.word_embeddings = self.load_data(
            dataset_path,
    ...
    def load_data(self, path, embedding_type='current', embedding_path=''):
        ...
        if embedding_type == 'fasttext':
            cache_path = f'{path}/word_embeddings_fasttext.npz'
            if os.path.exists(cache_path):
                word_embeddings = scipy.sparse.load_npz(cache_path).toarray().astype('float32')
            else:
                word_embeddings = self.make_fasttext_embeddings(vocab, embedding_path)
                scipy.sparse.save_npz(cache_path, word_embeddings)
        else:
    ...
    def make_fasttext_embeddings(self, vocab, embedding_path=''):
        if embedding_path:
            vector_path = embedding_path
        else:
            vector_path = './wordVector/cc.en.300.vec'

        try:
            fasttext_vectors = gensim.models.KeyedVectors.load_word2vec_format(vector_path, binary=False)
        except TypeError:
            fasttext_vectors = gensim.models.KeyedVectors.load_word2vec_format(vector_path, binary=False, no_header=False)
        word_embeddings = np.zeros((len(vocab), fasttext_vectors.vectors.shape[1]), dtype='float32')

        num_found = 0
        for i, word in enumerate(tqdm(vocab, desc="===>making fasttext embeddings")):
            if word in fasttext_vectors:
                word_embeddings[i] = fasttext_vectors[word]
                num_found += 1

        print(f'===> number of found fasttext embeddings: {num_found}/{len(vocab)}')
        return scipy.sparse.csr_matrix(word_embeddings)
